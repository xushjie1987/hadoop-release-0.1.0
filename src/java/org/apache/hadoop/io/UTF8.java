...
public class UTF8 implements WritableComparable {
    ...
    public void set(String string) {
        if (string.length() > 0xffff / 3) {             // maybe too long
            LOG.warning("truncating long string: " +
                        string.length() +
                        " chars, starting with " +
                        string.substring(0,
                                         20));
        ...
        if (bytes == null ||
            length > bytes.length)   // grow buffer
    ...
    public int compareTo(Object o) {
        UTF8 that = (UTF8) o;
        return WritableComparator.compareBytes(bytes,
                                               0,
                                               length,
                                               that.bytes,
                                               0,
    ...
    public int hashCode() {
        return WritableComparator.hashBytes(bytes,
                                            length);
    }
    ...
    public static class Comparator extends WritableComparator {
        ...
        public int compare(byte[] b1,
                           ...
                           int s2,
                           int l2) {
            int n1 = readUnsignedShort(b1,
                                       s1);
            int n2 = readUnsignedShort(b2,
                                       s2);
    ...
    static {                                        // register this comparator
        WritableComparator.define(UTF8.class,
                                  new Comparator());
    }
    ...
    private static void readChars(DataInput in,
                                  StringBuffer buffer,
