...
/***************************************************
 * FSNamesystem does the actual bookkeeping work for the DataNode. It tracks several important
 * tables. 1) valid fsname --> blocklist (kept on disk, logged) 2) Set of all valid blocks (inverted
 * #1) 3) block --> machinelist (kept in memory, rebuilt dynamically from reports) 4) machine -->
 * blocklist (inverted #2) 5) LRU cache of updated-heartbeat machines
 ***************************************************/
class FSNamesystem implements FSConstants {
    ...
    public FSNamesystem(File dir,
        ...
        this.desiredReplication = conf.getInt("dfs.replication",
                                              3);
        this.maxReplication = desiredReplication;
        this.maxReplicationStreams = conf.getInt("dfs.max-repl-streams",
                                                 2);
    ...
    /**
     * Close down this filesystem manager. Causes heartbeat and lease daemons to stop; waits briefly
     * for them to finish, but a short timeout returns control back to caller.
     */
    public void close() {
        synchronized (this) {
            fsRunning = false;
    ...
    // ///////////////////////////////////////////////////////
    /**
     * The client wants to open the given filename. Return a list of (block,machineArray) pairs. The
     * sequence of unique blocks in the list indicates all the blocks that make up the filename. The
     * client should choose one of the machines from the machineArray at random.
     */
    ...
    /**
     * The client would like to create a new block for the indicated filename. Return an array that
     * consists of the block, plus a set of machines. The first on this list should be where the
     * client writes data. Subsequent items in the list must be provided in the connection to the
     * first datanode.
     * 
     * @return Return an array that consists of the block, plus a set of machines, or null if src is
     *         invalid for creation (based on {@link FSDirectory#isValidToCreate(UTF8)}.
     */
    public synchronized Object[] startFile(UTF8 src,
        ...
        if (pendingCreates.get(src) == null) {
            ...
            LOG.warning("Cannot start file because pendingCreates is non-null. src=" +
                        src);
        }
        return results;
    }
    ...
    /**
     * The client would like to obtain an additional block for the indicated filename (which is
     * being written-to). Return an array that consists of the block, plus a set of machines. The
     * first on this list should be where the client writes data. Subsequent items in the list must
     * be provided in the connection to the first datanode. Make sure the previous blocks have been
     * reported by datanodes and are replicated. Will return an empty 2-elt array if we want the
     ...
     */
    public synchronized boolean abandonBlock(Block b,
                                             UTF8 src) {
        //
    ...
    /**
     * Finalize the created file and make it world-accessible. The FSNamesystem will already know
     * the blocks that make up the file. Before we return, we make sure that all the file's blocks
     * have been reported by datanodes and are replicated correctly.
     */
    public synchronized int completeFile(UTF8 src,
        ...
        if (dir.getFile(src) != null ||
            ...
            if (dir.addFile(src,
                ...
                System.out.println("AddFile() for " +
                                   src +
                                   " failed");
            }
    ...
    /**
     * Check that the indicated file's blocks are present and replicated. If not, return false.
     */
    synchronized boolean checkFileProgress(UTF8 src) {
        ...
        for (Iterator it = v.iterator(); it.hasNext();) {
            ...
            TreeSet containingNodes = (TreeSet) blocksMap.get(b);
            if (containingNodes == null ||
                containingNodes.size() < this.minReplication) {
                return false;
            }
        }
        return true;
    ...
    public boolean renameTo(UTF8 src,
                            UTF8 dst) {
        return dir.renameTo(src,
                            dst);
    }
    
    /**
     * Remove the indicated filename from the namespace. This may invalidate some blocks that make
     * up the file.
     */
                ...
                if (containingNodes != null) {
                    for (Iterator it = containingNodes.iterator(); it.hasNext();) {
                        ...
                        if (invalidateSet == null) {
                            ...
                            recentInvalidateSets.put(node.getName(),
                                                     invalidateSet);
                        }
                        invalidateSet.add(b);
                    }
    ...
    /**
     * Figure out a few hosts that are likely to contain the block(s) referred to by the given
     * (filename, start, len) tuple.
     */
    public UTF8[][] getDatanodeHints(UTF8 src,
        ...
        if (startBlock < 0 ||
            ...
            for (int i = startBlock; i <= endBlock; i++) {
                ...
                hosts[i -
                      startBlock] = (UTF8[]) v.toArray(new UTF8[v.size()]);
            }
            return hosts;
        }
    }
    ...
    /************************************************************
     * A Lease governs all the locks held by a single client. For each client there's a
     * corresponding lease, whose timestamp is updated when the client periodically checks in. If
     * the client dies and allows its lease to expire, all the corresponding locks can be released.
     *************************************************************/
    ...
    /******************************************************
     * LeaseMonitor checks for leases that have expired, and disposes of them.
     ******************************************************/
                    ...
                    synchronized (leases) {
                        ...
                        while ((sortedLeases.size() > 0) &&
                            ...
                            if (top.expired()) {
                                ...
                                LOG.info("Removing lease " +
                                         top +
                                         ", leases remaining: " +
                                         sortedLeases.size());
                                if (!sortedLeases.remove(top)) {
                                    LOG.info("Unknown failure trying to remove " +
                                             top +
                                             " from lease set.");
                                }
                            } else {
     ...
     */
    public synchronized int releaseLock(UTF8 src,
                                        UTF8 holder) {
        int result = internalReleaseLock(src,
                                         holder);
        if (result == COMPLETE_SUCCESS) {
            synchronized (leases) {
                Lease lease = (Lease) leases.get(holder);
    ...
    private int internalReleaseLock(UTF8 src,
                                    UTF8 holder) {
        return dir.releaseLock(src,
                               holder);
    }
    ...
    /**
     * Get a listing of all files at 'src'. The Object[] array exists so we can return file
     * attributes (soon to be implemented)
     */
    ...
    // ///////////////////////////////////////////////////////
    /**
     * The given node has reported in. This method should: 1) Record the heartbeat, so the datanode
     * isn't timed out 2) Adjust usage stats for future block allocation
     */
    public synchronized void gotHeartbeat(UTF8 name,
                                          long capacity,
                                          long remaining) {
        synchronized (heartbeats) {
            synchronized (datanodeMap) {
                ...
                if (nodeinfo == null) {
                    LOG.info("Got brand-new heartbeat from " +
                             name);
                    nodeinfo = new DatanodeInfo(name,
                                                capacity,
                                                remaining);
                    datanodeMap.put(name,
                                    nodeinfo);
                    capacityDiff = capacity;
                    remainingDiff = remaining;
                } else {
                    capacityDiff = capacity -
                                   nodeinfo.getCapacity();
                    remainingDiff = remaining -
                                    nodeinfo.getRemaining();
                    heartbeats.remove(nodeinfo);
                    nodeinfo.updateHeartbeat(capacity,
                                             remaining);
                }
    ...
    /**
     * Check if there are any expired heartbeats, and if so, whether any blocks have to be
     * re-replicated.
     */
            ...
            while ((heartbeats.size() > 0) &&
                ...
                if (deadblocks != null) {
                    for (int i = 0; i < deadblocks.length; i++) {
                        removeStoredBlock(deadblocks[i],
                                          nodeInfo);
                    }
                }
    ...
    /**
     * The given node is reporting all its blocks. Use this info to update the (machine-->blocklist)
     * and (block-->machinelist) tables.
     */
    public synchronized Block[] processReport(Block newReport[],
                                              UTF8 name) {
        DatanodeInfo node = (DatanodeInfo) datanodeMap.get(name);
        if (node == null) {
            throw new IllegalArgumentException("Unexpected exception.  Received block report from node " +
                                               name +
                                               ", but there is no info for " +
                                               name);
        }
        ...
        while (oldReport != null &&
               newReport != null &&
               oldPos < oldReport.length &&
               newPos < newReport.length) {
            ...
            if (cmp == 0) {
                ...
                removeStoredBlock(oldReport[oldPos],
                                  node);
                oldPos++;
            } else {
                // The new report has a block the old one does not
                addStoredBlock(newReport[newPos],
                               node);
                newPos++;
            }
        }
    ...
    /**
     * Modify (block-->datanode) map. Remove block from set of needed replications if this takes
     * care of the problem.
     */
    synchronized void addStoredBlock(Block block,
                                     DatanodeInfo node) {
        ...
        if (!containingNodes.contains(node)) {
            ...
            LOG.info("Redundant addStoredBlock request received for block " +
                     block +
                     " on node " +
                     node);
        }
        ...
        synchronized (neededReplications) {
            if (dir.isValidBlock(block)) {
                ...
                if (nonExcess.size() > this.maxReplication) {
                    chooseExcessReplicates(nonExcess,
                                           block,
                                           this.maxReplication);
                }
            }
    ...
    /**
     * We want a max of "maxReps" replicates for any block, but we now have too many. In this
     * method, copy enough nodes from 'srcNodes' into 'dstNodes' such that: srcNodes.size() -
     * dstNodes.size() == maxReps For now, we choose nodes randomly. In the future, we might enforce
     * some kind of policy (like making sure replicates are spread across racks).
     */
    void chooseExcessReplicates(Vector nonExcess,
                                Block b,
                                int maxReps) {
        while (nonExcess.size() -
               maxReps > 0) {
            ...
            if (excessBlocks == null) {
                ...
                excessReplicateMap.put(cur.getName(),
                                       excessBlocks);
            }
            excessBlocks.add(b);
            ...
            if (invalidateSet == null) {
                invalidateSet = new Vector();
                recentInvalidateSets.put(cur.getName(),
                                         invalidateSet);
            }
            invalidateSet.add(b);
        }
    ...
    /**
     * Modify (block-->datanode) map. Possibly generate replication tasks, if the removed block is
     * still valid.
     */
    synchronized void removeStoredBlock(Block block,
                                        DatanodeInfo node) {
        TreeSet containingNodes = (TreeSet) blocksMap.get(block);
        if (containingNodes == null ||
            !containingNodes.contains(node)) {
            throw new IllegalArgumentException("No machine mapping found for block " +
                                               block +
                                               ", which should be at node " +
                                               node);
        }
        ...
        //
        if (dir.isValidBlock(block) &&
            (containingNodes.size() < this.desiredReplication)) {
            synchronized (neededReplications) {
                neededReplications.add(block);
            }
        }
     ...
     */
    public synchronized void blockReceived(Block block,
                                           UTF8 name) {
        DatanodeInfo node = (DatanodeInfo) datanodeMap.get(name);
        if (node == null) {
    ...
    public DatanodeInfo[] datanodeReport() {
        ...
        synchronized (heartbeats) {
            synchronized (datanodeMap) {
                ...
                int i = 0;
                for (Iterator it = datanodeMap.values()
                                              .iterator(); it.hasNext();) {
                    DatanodeInfo cur = (DatanodeInfo) it.next();
                    results[i++] = cur;
                }
    ...
    /**
     * Return with a list of Block/DataNodeInfo sets, indicating where various Blocks should be
     * copied, ASAP. The Array that we return consists of two objects: The 1st elt is an array of
     * Blocks. The 2nd elt is a 2D array of DatanodeInfo objs, identifying the target sequence for
     * the Block at the appropriate index.
     */
        ...
        synchronized (neededReplications) {
            ...
            if (neededReplications.size() > 0) {
                ...
                for (Iterator it = neededReplications.iterator(); it.hasNext();) {
                    ...
                    //
                    if (scheduledXfers >= this.maxReplicationStreams -
                                          xmitsInProgress) {
                        break;
                    }
                    ...
                    if (!dir.isValidBlock(block)) {
                        ...
                        if (containingNodes.contains(srcNode)) {
                            DatanodeInfo targets[] = chooseTargets(Math.min(this.desiredReplication -
                                                                                    containingNodes.size(),
                ...
                if (replicateBlocks.size() > 0) {
                    ...
                    for (Iterator it = replicateBlocks.iterator(); it.hasNext(); i++) {
                        ...
                        if (containingNodes.size() +
                            targets.length >= this.desiredReplication) {
                            neededReplications.remove(block);
    ...
    /**
     * Get a certain number of targets, if possible. If not, return as many as we can.
     * 
     ...
     */
    DatanodeInfo[] chooseTargets(int desiredReplicates,
                                 TreeSet forbiddenNodes,
                                 UTF8 clientMachine) {
        TreeSet alreadyChosen = new TreeSet();
        Vector targets = new Vector();
        
        for (int i = 0; i < desiredReplicates; i++) {
            DatanodeInfo target = chooseTarget(forbiddenNodes,
                                               alreadyChosen,
                                               clientMachine);
            if (target != null) {
                targets.add(target);
    ...
    /**
     * Choose a target from available machines, excepting the given ones. Right now it chooses
     * randomly from available boxes. In future could choose according to capacity and
     * load-balancing needs (or even network-topology, to avoid inter-switch traffic).
     * 
     * @param forbidden1
     *            DatanodeInfo targets not allowed, null allowed.
     * @param forbidden2
     *            DatanodeInfo targets not allowed, null allowed.
     * @return DatanodeInfo instance to use or null if something went wrong (a log message is
     *         emitted if null is returned).
     */
    DatanodeInfo chooseTarget(TreeSet forbidden1,
                              TreeSet forbidden2,
                              UTF8 clientMachine) {
        //
