 ...
 * deployment can have one or many DataNodes. Each DataNode communicates regularly with a single
 * NameNode. It also communicates with client code and other DataNodes from time to time. DataNodes
 * store a series of named blocks. The DataNode allows client code to read these blocks, or to write
 * new block data. The DataNode may also, in response to instructions from its NameNode, delete
 * blocks or copy blocks to/from other DataNodes. The DataNode maintains just one critical table:
 * block-> stream of bytes (of BLOCK_SIZE or less) This info is stored on a local disk. The DataNode
 * reports the table's contents to the NameNode upon startup and every so often afterwards.
 * DataNodes spend their lives in an endless loop of asking the NameNode for something to do. A
 * NameNode cannot connect to a DataNode directly; a NameNode simply returns values from functions
 * invoked by a DataNode. DataNodes maintain an open server socket so that client code or other
 * DataNodes can read/write data. The host/port for this server is reported to the NameNode, which
 * then sends that information to clients or other DataNodes that might be interested.
 *
...
public class DataNode implements FSConstants, Runnable {
    ...
    public static InetSocketAddress createSocketAddr(String s) throws IOException {
        ...
        if (colonIndex < 0) {
            throw new RuntimeException("Not a host:port pair: " +
                                       s);
        }
        String host = target.substring(0,
                                       colonIndex);
    ...
    /**
     * Create the DataNode given a configuration and a dataDir. 'dataDir' is where the blocks are
     * stored.
     */
    ...
    public DataNode(String machineName,
        ...
        int tmpPort = conf.getInt("dfs.datanode.port",
                                  50010);
        while (ss == null) {
            try {
                ss = new ServerSocket(tmpPort);
                LOG.info("Opened server at " +
                         tmpPort);
            } catch (IOException ie) {
                LOG.info("Could not open server at " +
                         tmpPort +
                         ", trying new port");
                tmpPort++;
            }
        }
        this.localName = machineName +
                         ":" +
                         tmpPort;
        ...
        this.blockReportInterval = blockReportIntervalBasis -
                                   new Random().nextInt((int) (blockReportIntervalBasis / 10));
    ...
    public void offerService() throws Exception {
        ...
        LOG.info("using BLOCKREPORT_INTERVAL of " +
                 blockReportInterval +
                 "msec");
        ...
        while (shouldRun) {
            ...
            synchronized (receivedBlockList) {
                if (now -
                    ...
                    // -- Bytes remaining
                    //
                    namenode.sendHeartbeat(localName,
                                           data.getCapacity(),
                                           data.getRemaining());
                ...
                if (now -
                    lastBlockReport > blockReportInterval) {
                    //
                    // Send latest blockinfo report if timer has expired.
                    // Get back a list of local block(s) that are obsolete
                    // and can be safely GC'ed.
                    //
                    Block toDelete[] = namenode.blockReport(localName,
                                                            data.getBlockReport());
                ...
                if (receivedBlockList.size() > 0) {
                    ...
                    namenode.blockReceived(localName,
                                           blockArray);
                }
                ...
                //
                if (now -
                    sendStart > datanodeStartupPeriod) {
                    //
                        ...
                        for (int i = 0; i < blocks.length; i++) {
                            if (!data.isValidBlock(blocks[i])) {
                                String errStr = "Can't send invalid block " +
                                                blocks[i];
                                LOG.info(errStr);
                                namenode.errorReport(localName,
                                                     errStr);
                                break;
                            } else {
                                if (xferTargets[i].length > 0) {
                                    LOG.info("Starting thread to transfer block " +
                                             blocks[i] +
                                             " to " +
                                             xferTargets[i]);
                                    new Daemon(new DataTransfer(xferTargets[i],
                                                                blocks[i])).start();
                                }
                            }
                        }
                    } else if (cmd != null &&
                               cmd.invalidateBlocks()) {
                        //
                ...
                //
                long waitTime = HEARTBEAT_INTERVAL -
                                (now - lastHeartbeat);
                if (waitTime > 0 &&
                    receivedBlockList.size() == 0) {
                    try {
    ...
    /**
     * Server used for receiving/sending a block of data. This is created to listen for requests
     * from clients or other DataNodes. This small server does not use the Hadoop IPC mechanism.
     */
    class DataXceiveServer implements Runnable {
        ...
        public void run() {
            try {
                ...
                LOG.info("Exiting DataXceiveServer due to " +
                         ie.toString());
            }
        }
                    ...
                    if (op == OP_WRITE_BLOCK) {
                        ...
                        try {
                            ...
                            LocatedBlock newLB = new LocatedBlock(b,
                                                                  (DatanodeInfo[]) mirrors.toArray(new DatanodeInfo[mirrors.size()]));
                            newLB.write(reply);
                        } finally {
                            reply.close();
                        }
                    } else if (op == OP_READ_BLOCK ||
                               op == OP_READSKIP_BLOCK) {
                        //
                        // Read in the header
                        //
                        Block b = new Block();
    ...
    /**
     * Used for transferring a block of data. This class sends a piece of data to another DataNode.
     */
    class DataTransfer implements Runnable {
        ...
        /**
         * Connect to the first item in the target list. Pass along the entire target list, the
         * block, and the data.
         */
        public DataTransfer(DatanodeInfo targets[],
    ...
    /**
     * No matter what kind of exception we get, keep retrying to offerService(). That's the loop
     * that connects to the NameNode and provides basic DataNode functionality. Only stop when
     * "shouldRun" is turned off (which can only happen at shutdown).
     */
    public void run() {
        LOG.info("Starting DataNode in: " +
                 data.data);
        while (shouldRun) {
            try {
                offerService();
        ...
        LOG.info("Finishing DataNode in: " +
                 data.data);
    }
    
    /**
     * Start datanode daemons. Start a datanode daemon for each comma separated data directory
    ...
    /**
     * Start datanode daemons. Start a datanode daemon for each comma separated data directory
     * specified in property dfs.data.dir and wait for them to finish. If this thread is
    ...
    /**
     * Make an instance of DataNode after ensuring that given data directory (and parent
     * directories, if necessary) can be created.
     * 
     * @param dataDir
     *            where the new DataNode instance should keep its files.
     * @param conf
     *            Configuration instance to use.
     * @return DataNode instance for given data dir and conf, or null if directory cannot be
     *         created.
     * @throws IOException
     */
    static DataNode makeInstanceForDir(String dataDir,
        ...
        data.mkdirs();
        if (!data.isDirectory()) {
            LOG.warning("Can't start DataNode in non-directory: " +
                        dataDir);
            return null;
        } else {
            dn = new DataNode(conf,
                              dataDir);
        }
        return dn;
    }
