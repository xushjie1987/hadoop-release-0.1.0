 ...
 * is a single NameNode running in any DFS deployment. (Well, except when there is a second
 * backup/failover NameNode.) The NameNode controls two critical tables: 1) filename->blocksequence
 * (namespace) 2) block->machinelist ("inodes") The first table is stored on disk and is very
 * precious. The second table is rebuilt every time the NameNode comes up. 'NameNode' refers to both
 * this class as well as the 'NameNode server'. The 'FSNamesystem' class actually performs most of
 * the filesystem management. The majority of the 'NameNode' class itself is concerned with exposing
 * the IPC interface to the outside world, plus some configuration management. NameNode implements
 * the ClientProtocol interface, which allows clients to ask for DFS services. ClientProtocol is not
 * designed for direct use by authors of DFS client code. End-users should instead use the
 * org.apache.nutch.hadoop.fs.FileSystem class. NameNode also implements the DatanodeProtocol
 * interface, used by DataNode programs that actually store DFS data blocks. These methods are
 * invoked repeatedly and automatically by all the DataNodes in a DFS deployment.
 *
...
public class NameNode implements ClientProtocol, DatanodeProtocol, FSConstants {
    ...
    public NameNode(File dir,
        ...
        this.namesystem = new FSNamesystem(dir,
                                           conf);
        this.handlerCount = conf.getInt("dfs.namenode.handler.count",
                                        10);
    ...
    private static File getDir(Configuration conf) {
        return new File(conf.get("dfs.name.dir",
                                 "/tmp/hadoop/dfs/name"));
    }
    ...
    /**
     * Stop all NameNode threads and wait for all to finish. Package-only access since this is
     * intended for JUnit testing.
     */
    void stop() {
        if (!stopRequested) {
            stopRequested = true;
    ...
    public LocatedBlock[] open(String src) throws IOException {
        ...
        if (openResults == null) {
            ...
            for (int i = 0; i < blocks.length; i++) {
                results[i] = new LocatedBlock(blocks[i],
                                              sets[i]);
            }
            return results;
        }
    ...
    public LocatedBlock addBlock(String src,
        ...
        if (results == null) {
            throw new IOException("Cannot obtain additional block for file " +
                                  src);
    ...
    /**
     * The client can report in a set written blocks that it wrote. These blocks are reported via
     * the client instead of the datanode to prevent weird heartbeat race conditions.
     */
    public void reportWrittenBlock(LocatedBlock lb) throws IOException {
        Block b = lb.getBlock();
        DatanodeInfo targets[] = lb.getLocations();
        for (int i = 0; i < targets.length; i++) {
            namesystem.blockReceived(b,
                                     targets[i].getName());
        }
    }
    ...
    public boolean complete(String src,
        ...
        int returnCode = namesystem.completeFile(new UTF8(src),
                                                 new UTF8(clientName));
    ...
    public String[][] getHints(String src,
        ...
        UTF8 hosts[][] = namesystem.getDatanodeHints(new UTF8(src),
                                                     start,
                                                     len);
        if (hosts == null) {
            return new String[0][];
    ...
    public boolean rename(String src,
        ...
        return namesystem.renameTo(new UTF8(src),
                                   new UTF8(dst));
    }
    ...
    public boolean obtainLock(String src,
        ...
        int returnCode = namesystem.obtainLock(new UTF8(src),
                                               new UTF8(clientName),
                                               exclusive);
        if (returnCode == COMPLETE_SUCCESS) {
            ...
            throw new IOException("Failure when trying to obtain lock on " +
                                  src);
        }
    }
    ...
    public boolean releaseLock(String src,
                               String clientName) throws IOException {
        int returnCode = namesystem.releaseLock(new UTF8(src),
                                                new UTF8(clientName));
        if (returnCode == COMPLETE_SUCCESS) {
            return true;
        } else if (returnCode == STILL_WAITING) {
            ...
            throw new IOException("Failure when trying to release lock on " +
                                  src);
        }
    }
    ...
    public long[] getStats() throws IOException {
        ...
        results[1] = namesystem.totalCapacity() -
                     namesystem.totalRemaining();
        return results;
    }
    ...
    public void sendHeartbeat(String sender,
                              long capacity,
                              long remaining) {
        namesystem.gotHeartbeat(new UTF8(sender),
                                capacity,
                                remaining);
    }
    
    public Block[] blockReport(String sender,
                               Block blocks[]) {
        LOG.info("Block report from " +
                 sender +
                 ": " +
                 blocks.length +
                 " blocks.");
        return namesystem.processReport(blocks,
                                        new UTF8(sender));
    }
    ...
    public void blockReceived(String sender,
                              Block blocks[]) {
        for (int i = 0; i < blocks.length; i++) {
            namesystem.blockReceived(blocks[i],
                                     new UTF8(sender));
        }
    }
    ...
    /**
     * Return a block-oriented command for the datanode to execute. This will be either a transfer
    ...
    public static void main(String argv[]) throws Exception {
        ...
        if (argv.length == 1 &&
            argv[0].equals("-format")) {
            File dir = getDir(conf);
            if (dir.exists()) {
                System.err.print("Re-format filesystem in " +
                                 dir +
                                 " ? (Y or N) ");
                if (!(System.in.read() == 'Y')) {
