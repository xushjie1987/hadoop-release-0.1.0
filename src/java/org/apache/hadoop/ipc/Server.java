...
/**
 * An abstract IPC service. IPC calls take a single {@link Writable} as a parameter, and return a
 * {@link Writable} as their value. A service runs on a port and is defined by a parameter class and
 * a value class.
 * 
 * @author Doug Cutting
 * @see Client
...
public abstract class Server {
    ...
    /**
     * Returns the server instance called under or null. May be called under {@link #call(Writable)}
     * implementations, and under {@link Writable} methods of paramters and return values. Permits
     * applications to access the server context.
     */
    ...
    private class Connection extends Thread {
        ...
        public Connection(Socket socket) throws IOException {
            ...
            this.setDaemon(true);
            this.setName("Server connection on port " +
                         port +
                         " from " +
        ...
        public void run() {
            ...
            while (running) {
                try {
                    ...
                    synchronized (callQueue) {
                        while (running &&
                               callQueue.size() == 0) { // wait for a call
    ...
    /**
     * Constructs a server listening on the named port. Parameters passed must be of the named
     * class. The <code>handlerCount</handlerCount> determines
    ...
    /**
     * Stops the service. No new calls will be handled after this is called. All subthreads will
     * likely be finished after this returns.
     */
    public synchronized void stop() {
        LOG.info("Stopping server on " +
                 port);
        running = false;
        try {
    ...
    /**
     * Wait for the server to be stopped. Does not wait for all subthreads to finish. See
     * {@link #stop()}.
     */
