...
/**
 * A client for an IPC service. IPC calls take a single {@link Writable} as a parameter, and return
 * a {@link Writable} as their value. A service runs on a port and is defined by a parameter class
 * and a value class.
 * 
 * @author Doug Cutting
 * @see Server
...
public class Client {
    ...
    private Class              valueClass;                                                          // class
                                                                                                     // of
                                                                                                     // call
                                                                                                     // values
    ...
    private int                counter;                                                             // counter
                                                                                                     ...
                                                                                                     // call
                                                                                                     // ids
    private boolean            running     = true;                                                  // true
                                                                                                     // while
                                                                                                     // client
                                                                                                     // runs
    ...
    private class Call {
        ...
        /**
         * Called by the connection thread when the call is complete and the value or error string
         * are available. Notifies by default.
         */
    ...
    /**
     * Thread that reads responses and notifies callers. Each connection owns a socket connected to
     * a remote address. Calls are multiplexed through this socket: responses may be delivered out
     * of order.
     */
    private class Connection extends Thread {
        ...
        /**
         * Initiates a call by sending the parameter to the remote server. Note: this is not called
         * from the Connection thread, but by other threads.
         */
        public void sendParam(Call call) throws IOException {
            boolean error = true;
            try {
                calls.put(new Integer(call.id),
                          call);
                synchronized (out) {
                    if (LOG.isLoggable(Level.FINE))
                        LOG.fine(getName() +
    ...
    private class ParallelCall extends Call {
        ...
        public ParallelCall(Writable param,
    ...
    public Client(Class valueClass,
        ...
        this.timeout = conf.getInt("ipc.client.timeout",
                                   10000);
        this.conf = conf;
    }
     ...
     * returning the value. Throws exceptions if there are network problems or if the remote code
     * threw an exception.
     */
    ...
    /**
     * Makes a set of calls in parallel. Each parameter is sent to the corresponding address. When
     * all values are available, or have timed out or errored, the collected results are returned in
     * an array. The array contains nulls for calls that timed out or errored.
     */
    public Writable[] call(Writable[] params,
        ...
        synchronized (results) {
            for (int i = 0; i < params.length; i++) {
                ParallelCall call = new ParallelCall(params[i],
                                                     results,
                                                     i);
                try {
