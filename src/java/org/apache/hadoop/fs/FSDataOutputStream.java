...
/**
 * Utility that wraps a {@link FSOutputStream} in a {@link DataOutputStream}, buffers output through
 * a {@link BufferedOutputStream} and creates a checksum file.
 */
public class FSDataOutputStream extends DataOutputStream {
    ...
    private static class Summer extends FilterOutputStream {
        ...
        public Summer(FileSystem fs,
            ...
            super(fs.createRaw(file,
                               overwrite));
            this.bytesPerSum = conf.getInt("io.bytes.per.checksum",
                                           512);
            this.sums = new FSDataOutputStream(fs.createRaw(fs.getChecksumFile(file),
                                                            true),
                                               conf);
            
            sums.write(CHECKSUM_VERSION,
                       0,
    ...
    public FSDataOutputStream(FileSystem fs,
                              ...
                              Configuration conf,
                              int bufferSize) throws IOException {
        super(new Buffer(new PositionCache(new Summer(fs,
                                                      file,
                                                      overwrite,
                                                      conf)),
                         bufferSize));
    ...
    private FSDataOutputStream(FSOutputStream out,
                               Configuration conf) throws IOException {
        this(out,
    ...
    private FSDataOutputStream(FSOutputStream out,
                               int bufferSize) throws IOException {
        super(new Buffer(new PositionCache(out),
                         bufferSize));
    }
