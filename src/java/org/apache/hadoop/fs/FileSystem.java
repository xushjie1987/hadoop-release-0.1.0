...
public abstract class FileSystem extends Configured {
    ...
    /**
     * Parse the cmd-line args, starting at i. Remove consumed args from array. We expect param in
     * the form: '-local | -dfs <namenode:port>'
     *
    ...
    public static FileSystem parseArgs(String argv[],
        ...
        if ("-dfs".equals(cmd)) {
            ...
            fs = new DistributedFileSystem(addr,
                                           conf);
            ...
            LOG.info("No FS indicated, using default:" +
                     fs.getName());
            
        }
        System.arraycopy(argv,
                         i,
                         argv,
                         orig,
                         argv.length -
                                 i);
        for (int j = argv.length -
                     i; j < argv.length; j++) {
            argv[j] = null;
        }
        return fs;
    ...
    public static FileSystem get(Configuration conf) throws IOException {
        return getNamed(conf.get("fs.default.name",
                                 "local"),
                        conf);
    }
     ...
     * {@link FileSystem#getNamed(String,Configuration)}.
     */
    public abstract String getName();
    
    /**
     * Returns a named filesystem. Names are either the string "local" or a host:port pair, naming
     * an DFS name server.
     */
    public static FileSystem getNamed(String name,
        ...
        if (fs == null) {
            if ("local".equals(name)) {
                ...
                fs = new DistributedFileSystem(DataNode.createSocketAddr(name),
                                               conf);
            }
            NAME_TO_FS.put(name,
                           fs);
    ...
    /**
     * Return a 2D array of size 1x1 or greater, containing hostnames where portions of the given
     * file can be found. For a nonexistent file or regions, null will be returned. This call is
     * most helpful with DFS, where it returns hostnames of machines that contain the given file.
    ...
    public FSDataInputStream open(File f,
        ...
        return new FSDataInputStream(this,
                                     f,
                                     bufferSize,
                                     getConf());
    }
    ...
    public FSDataOutputStream create(File f) throws IOException {
        return create(f,
                      true,
                      getConf().getInt("io.file.buffer.size",
                                       4096));
    }
    ...
    public FSDataOutputStream create(File f,
                                     boolean overwrite,
                                     int bufferSize) throws IOException {
        return new FSDataOutputStream(this,
                                      f,
                                      overwrite,
                                      getConf(),
                                      bufferSize);
    ...
    /**
     * Creates the given File as a brand-new zero-length file. If create fails, or if it already
    ...
    public boolean rename(File src,
        ...
        if (isDirectory(src)) {
            return renameRaw(src,
                             dst);
        } else {
            
            boolean value = renameRaw(src,
                                      dst);
            
            File checkFile = getChecksumFile(src);
            if (exists(checkFile))
                renameRaw(checkFile,
                          getChecksumFile(dst)); // try to rename checksum
    ...
    /**
     * The src file is on the local disk. Add it to FS at the given dst name, removing the source
     * afterwards.
     */
    ...
    /**
     * The src file is under FS2, and the dst is on the local disk. Copy it from FS control to the
     * local dst name.
     */
    ...
    /**
     * the same as copyToLocalFile(File src, File dst), except that the source is removed afterward.
     */
    // not implemented yet
    // public abstract void moveToLocalFile(File src, File dst) throws IOException;
    
    /**
     * Returns a local File that the user can write output to. The caller provides both the eventual
     * FS target name and the local working file. If the FS is local, we write directly into the
     * target. If the FS is remote, we write into the tmp local area.
     */
    public abstract File startLocalOutput(File fsOutputFile,
                                          File tmpLocalFile) throws IOException;
    
    /**
     * Called when we're all done writing to the target. A local FS will do nothing, because we've
     * written to exactly the right place. A remote FS will copy the contents of tmpLocalFile to the
     * correct target at fsOutputFile.
     */
    public abstract void completeLocalOutput(File fsOutputFile,
                                             File tmpLocalFile) throws IOException;
    
    /**
     * Returns a local File that the user can read from. The caller provides both the eventual FS
     * target name and the local working file. If the FS is local, we read directly from the source.
     * If the FS is remote, we write data into the tmp local area.
     */
     ...
     * @param length
     *            the length of the bad data in the file
     * @param crc
     *            the expected CRC32 of the data
     */
