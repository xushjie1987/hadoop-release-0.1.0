...
/**
 * Utility that wraps a {@link FSInputStream} in a {@link DataInputStream} and buffers input through
...
public class FSDataInputStream extends DataInputStream {
    ...
    private class Checker extends FilterInputStream implements Seekable {
        ...
        public Checker(FileSystem fs,
            ...
            try {
                this.sums = new FSDataInputStream(fs.openRaw(sumFile),
                                                  conf);
                ...
                LOG.warning("Problem opening checksum file: " +
                            file +
                            ".  Ignoring with exception " +
                            e +
                            ".");
                stopSumming();
            }
        ...
        public void seek(long desired) throws IOException {
            ...
            if (sums != null) {
                ...
                try {
                    sums.seek(HEADER_LENGTH +
                              4 *
                              (desired / bytesPerSum));
                } catch (IOException e) {
                    LOG.warning("Problem seeking checksum file: " +
                                e +
                                ". Ignoring.");
                    stopSumming();
                }
        ...
        public int read(byte b[],
            ...
            if (sums != null) {
                ...
                while (summed < read) {
                    ...
                    int goal = bytesPerSum -
                               inSum;
                    int inBuf = read -
                                summed;
                    int toSum = inBuf <= goal
                                             ? inBuf
                                             : goal;
        ...
        private void verifySum(int delta) throws IOException {
            ...
            try {
                ...
                LOG.warning("Problem reading checksum file: " +
                            e +
                            ". Ignoring.");
                stopSumming();
                return;
    ...
    private class Buffer extends BufferedInputStream {
        public Buffer(PositionCache in,
        ...
        public long getPos() throws IOException {     // adjust for buffer
            return ((PositionCache) in).getPos() -
                   (this.count - this.pos);
        }
    ...
    public FSDataInputStream(FileSystem fs,
                             ...
                             Configuration conf) throws IOException {
        super(null);
        this.in = new Buffer(new PositionCache(new Checker(fs,
                                                           file,
                                                           conf)),
                             bufferSize);
    }
    ...
    public FSDataInputStream(FSInputStream in,
                             int bufferSize) throws IOException {
        super(null);
        this.in = new Buffer(new PositionCache(in),
                             bufferSize);
    }
